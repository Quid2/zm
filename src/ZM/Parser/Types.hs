...
{-# LANGUAGE MultiParamTypeClasses ,PatternSynonyms ,RankNTypes #-}
...
module ZM.Parser.Types
    ...
    , Val,ValF(..)
    , pattern Constr
     ,pattern VInteger,pattern VChar,pattern VString,pattern VFloat,pattern VArray,pattern PBind,pattern PWild
    , Fix(..)
    , Annotate(..)
    , Literal(..)
    , Binder(..)
    , Value,pattern Value,valType,valName,valBits,valFields
    , Void, Void1
    , module Data.Either.Validation) where
...
-- |A parsed ADT
data ADTParts = ADTParts { name :: AtAbsName
                         , vars :: [AtId]
...
data ValF lit binder r =
    ConstrF 
      String -- ^Name of the constructor (e.g. "True")
      (Either [r] [(String,r)]) -- ^Constructor parameters, possibly named
    --        } -- ^A standard constructor, e.g. PCon "True" []

  | LitF (lit r) -- ^A variable or a lit pattern (e.g. a string or a number)
  | BinderF binder -- ^A pattern that might bind a matched value to a name
  -- deriving (Eq, Ord, Show)
...
data Literal e = LInteger Integer
...
pattern Value :: forall a b lit binder.
                       a
                       -> String
                       -> b
