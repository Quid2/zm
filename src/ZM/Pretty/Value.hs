  ...
  pPrintPrec (PrettyLevel lev) _ = pp 0
   where -- TODO: fix precedence
    ...
    pp l v@(Value t n bs vs) =
      ...
      in  (if complex && l > 0 then parens else id)
            ( hsep
            $ (if null bs || lev == 0
                then empty
                else
                  (text (map (\b -> if b then '1' else '0') bs) P.<> char ':')
              )
            : doc
            ) -- : if lev == 0 then empty else )

    ch (Value _ _ _ [w]) = chr $ wl_ w
    ch v                 = error (show v)

    -- wrd = int . wrd_
    wrd = int . fromIntegral . wrd_

    wrd_ (Value _ ('V' : n) _ _) = read n :: Word8 -- Int -- Word64?
    ...
    -- i p = (absType p,\v -> (False,int . (zagZig::(Bits a,Integral a) => Word64 -> a) . wl_ . p0 . p0 $ v))

    wl = text . show . (wl_ :: Value -> Word64)

    wl_ (Value _ _ _ [Value _ _ _ [Value _ _ _ [vl]]]) =
      fromIntegral
        . fst
        . foldl (\(t, e) n -> (t + n * 2 ^ e, e + 7)) (0 :: Int, 0 :: Int)
        . map wrd2_
        . neList
        $ vl
    ...
    arrList (Value _ ('A' : n) _ vs) | length vs == read n + 1 =
      init vs ++ arrList (last vs)
    ...
    prettyPrinter t =
      listToMaybe
        ...
        $ [ (absType (Proxy :: Proxy ())    , \_ -> (False, text "()"))
          ...
          , ( absType (Proxy :: Proxy Int8)
            , \v -> (False, int . zagZig . wrd_ . p0 . p0 $ v)
            )
          ...
          , ( absType (Proxy :: Proxy Integer)
            , \v ->
              ( False
              , int . fromIntegral . zagZig . (wl_ :: Value -> Natural) . p0 $ v
              )
            )
      --,(absType (Proxy::Proxy Natural),\v -> (False,int . fromIntegral . zagZigInteger . wl_ . p0 $ v))
          , ( absType (Proxy :: Proxy Float)
            , \v -> (False, float $ floatVal v)
            ) -- (False,int . fromIntegral . zagZigInteger . wl_ . p0 $ v))
          , ( absType (Proxy :: Proxy Double)
            , \v -> (False, double $ doubleVal v)
            ) -- (False,int . fromIntegral . zagZigInteger . wl_ . p0 $ v))
      --,(absType (Proxy::Proxy Char),\v -> (False,text ['\'',ch v,'\'']))
          , ( absType (Proxy :: Proxy Char)
            , \v -> (False, pPrint (ch v))
            )
      --,(absType (Proxy::Proxy [Char]),\v -> (False,pPrint . map ch . valList $ v))
          , ( absType (Proxy :: Proxy [Char])
            , \v -> (False, text . show . map ch . valList $ v)
            )
      --,(absType (Proxy::Proxy ([Char])),\v -> (False,char '"' <> (text . map ch . valList $ v) <> char '"'))
          , (absType (Proxy :: Proxy [Any]), \v -> (False, ar (valList v)))
          , ( absType (Proxy :: Proxy (NonEmptyList Any))
            , \v -> (False, ar (neList v))
            )
          , ( absType (Proxy :: Proxy (S.Seq Char))
            , \v -> (False, pPrint . map ch . arrList $ v)
            )
          , ( absType (Proxy :: Proxy (S.Seq Any))
            , \v -> (False, arr (arrList v))
            )
      ...
      --,(absType (Proxy::Proxy (P.Array Any)),\v -> (False,arr (arrList v)))
          ...
          , ( absType (Proxy :: Proxy (Any, Any, Any, Any, Any, Any, Any))
            , tuple
            )
          , ( absType (Proxy :: Proxy (Any, Any, Any, Any, Any, Any, Any, Any))
            , tuple
            )
          , ( absType
              (Proxy :: Proxy (Any, Any, Any, Any, Any, Any, Any, Any, Any))
            , tuple
            )
          ]
     where
      ...
      bytes bs =
        let [_, vs] = valFields . head . valFields $ bs
      ...
      floatVal (Value { valName = "IEEE_754_binary32", valFields = [Value { valBits = [signVal] }, msb -> expVal, msb -> frac] })
        = ieee signVal [bits8 expVal] [bits23 frac] 127

      doubleVal (Value { valName = "IEEE_754_binary64", valFields = [Value { valBits = [signVal] }, msb -> expVal, msb -> frac] })
        = ieee signVal [bits11 expVal] [bits52 frac] 1023

      ieee sign exps fracs expOff =
        let
          signV    = fromIntegral $ fromEnum sign
          expV     = fromIntegral $ bitsVal $ concat exps
          fracBits = concat $ [True] : fracs
          fracV    = fromIntegral $ bitsVal fracBits
          val =
            ((-1) ** signV)
              * (fracV / (2 ^ (length fracBits - 1)))
              * (2 ** (expV - expOff))
        in
          val
...
bitsVal =
  fst
    . foldl (\(t, e) n -> (t + fromEnum n * 2 ^ e, e + 1)) (0 :: Int, 0 :: Int)
    . reverse
